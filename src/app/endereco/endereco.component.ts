...
export class EnderecoComponent extends BaseEtapaComponent<Endereco> {
  estadosLista: any[];
  municipioLista: any[];
  ...
  iniciar() {
    ...
    this.formulario.get('estado').valueChanges.subscribe(id => {
      this.service.Get<Municipio[]>('municipio/BuscarPorIdEstado', id).subscribe(object => this.municipioLista = object);
    });
